...
T_e = 1385.84 * K_1 * Omega_s**(-1)		# Electron temperature
print("Electron Temperature (K): ", T_e)

n_e = sqrt( 1710.0 * K_2 * (T_e)**(1.35) * D**(-1) * theta_min**(-1) )
print("Electron density (cm^-3): ", n_e)

freq_c = K_2**(1/2.1)				# Turnover freq. in GHz
...
plt.plot(np.log10(freq_samples), np.log10((K_1*freq_samples**2)*(1 - np.exp(-K_2*freq_samples**-2.1))), 'r-', label='Free-Free')
...
print("Flux at 3.6 cm (8.3 GHz): ", 10**combined_power_law_fit(8.3, alpha_high, K_1, K_2, K_3))
print("Flux at 2.7 mm (111 GHz): ", 10**combined_power_law_fit(111.0, alpha_high, K_1, K_2, K_3))
...
plt.savefig('spectrum_L1551_IRS_5_S_ff_spec_fit_with_1998_data.png')
